...
export default function TabBody({header, data = [] , onClick ,statusFilter,onSort , groupBy = false , onGroupDelete }) {
  ...
  if(data.length === 0){
    if(statusFilter === "all")
      return <h5 style={{textAlign: 'center', paddingTop: '200px'}}>There are no tasks!</h5>
    if(statusFilter === "open")
      return <h5 style={{textAlign: 'center', paddingTop: '200px'}}>There are no pending tasks!</h5>
    if(statusFilter === "done")
      return <h5 style={{textAlign: 'center', paddingTop: '200px'}}>There are no completed tasks!</h5>
  }
  const onDelete = () => {
    onGroupDelete(state)
  }
